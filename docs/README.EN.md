Let's start small, how to screw the `Android NDK` to` C :: B`
 ...
 - 1. register a profile for each platform, with executable files according to the platform.  In my opinion it is tiring, a lot of things and it is not clear why.  Given that if you collect the same for all platforms, the volume of gestures with switching configurations is quite large.
 My default is to build arm64-v8a, armeabi-v7a, x86, x86_64.
 ...
 - full integration of the `C :: B` projects when using the export utility [cpb2ndk] (CBP2NDK.md).

 ### Android NDK Integration

 ----------

 - __The first way __: download and install the ready installer `NDK C :: B template`, [more ..] (CBNDKAUTOINSTALL.md)
 - __Path two __: in manual mode, set `NDK C :: B template` for each project, as described below.

 ----------

 The ideology and manner of building an application using the Android Android NDK toolchain is as close as possible to the typical behavior of `C :: B`, the process logic:
...
     WARNING: linker: / data / local / tmp / youprog: unused DT entry: type 0x6fffffff arg 0x2
 ...
 The full code `NDK C :: B template` is located in the [C :: B template] directory (https://github.com/ClnViewer/Code-Blocks-Android-NDK/tree/master/CB-Template/)
 The [installer] page (CBNDKAUTOINSTALL.md) `NDK C :: B template`
